...
public class ActivityController {
	...
	public R<List<Activity>> hot(
			@Parameter(description = "返回数量") @RequestParam(defaultValue = "10") int limit,
			@Parameter(description = "地区ID") @RequestParam(required = false) Long regionId) {
		return R.ok(activityService.getHot(limit, regionId));
	}
	...
	public R<List<Activity>> recommended(
			@Parameter(description = "返回数量") @RequestParam(defaultValue = "10") int limit,
			@Parameter(description = "地区ID") @RequestParam(required = false) Long regionId) {
		return R.ok(activityService.getRecommended(limit, regionId));
	}
	...
	@Operation(summary = "按年月查询活动（日历视图）")
	@GetMapping("/calendar")
	public R<List<Activity>> calendar(
			@Parameter(description = "年份") @RequestParam int year,
			@Parameter(description = "月份") @RequestParam int month,
			@Parameter(description = "地区ID") @RequestParam(required = false) Long regionId) {
		return R.ok(activityService.getByMonth(year, month, regionId));
	}
}
